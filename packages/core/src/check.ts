...
export async function checkProviders(env: Record<string, string | undefined> = process.env): Promise<CheckResult[]> {
  ...
  for (const [id, provider] of Object.entries(providers)) {
    ...
    results.push(
      await probe("search", id, async (signal) => {
        const items = await provider.search(QUERY, { limit: 3, signal, key, filters });
  ...
  for (const [id, fetcher] of Object.entries(fetchers)) {
    const result = await probeFetcher(id, fetcher, env);
    if (result) results.push(result);
  }
  return results;
}

async function probeFetcher(id: string, fetcher: Fetcher, env: Record<string, string | undefined>): Promise<CheckResult | undefined> {
  ...
  return probe("fetch", id, async (signal) => {
    const page = await fetcher.fetch(PAGE, { format: "markdown", maxCharacters: 50_000, signal, key });
